...
イシューをもとに作業ステップを立案し、ステップごとに実行する。task指定があれば該当する作業のみをステップに切り出すこと。

各ステップは下記の流れを厳守：
1. 該当するtaskを定める（task指定されていない場合）
2. 該当する`.claude/works/commons-{task}.md`があれば念入りに読み込む
3. 作業を実施。判断が必要なところはユーザーに確認を取ること
4. ステップ完了ごとにコミット（メッセージ: `{task}: ステップ名`）
...
/scrap_builder web_design # イシューに対応するデザイン作業の実行
/scrap_builder web_design ダークモード  # taskを依頼を考慮して実行
```
